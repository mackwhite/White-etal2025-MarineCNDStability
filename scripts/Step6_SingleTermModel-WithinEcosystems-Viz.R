...
mean(p_synch$`r_Program[PCCS,synch]` + p_synch$b_synch < 0)
mean(p_synch$`r_Program[MCR,synch]` + p_synch$b_synch < 0)
mean(p_synch$`r_Program[FCE,synch]` + p_synch$b_synch < 0)
...
mean(p_sr$`r_Program[VCR,mean_species_richness]` + p_sr$b_mean_species_richness > 0)
mean(p_sr$`r_Program[PCCS,mean_species_richness]` + p_sr$b_mean_species_richness > 0)
mean(p_sr$`r_Program[MCR,mean_species_richness]` + p_sr$b_mean_species_richness > 0)
mean(p_sr$`r_Program[FCE,mean_species_richness]` + p_sr$b_mean_species_richness > 0)
...
prog = c('PCCC', 'VCR', 'SBC', 'PCCS', 'FCE', 'MCR', 'Overall')
...
b = ggplot(df_beta|> filter(Program != 'Overall'), aes(Program, value, color = Program))+
      ...
      scale_color_manual(values = program_palette)+
      scale_y_continuous(limits = c(-0.35, 1.03))+
      annotate('text', x = 2, y = 0.8, size = 4.0,
               label = bquote(atop(
                     {R^2}[cond] ~ '= 0.54',
                     {R^2}[mar] ~ '= 0.04 ')))+
      coord_flip()+
